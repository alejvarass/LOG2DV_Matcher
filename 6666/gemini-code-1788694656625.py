"""
Lith_main.py – Analizador Litológico de Patrones (Enfoque Híbrido Integral con IA)
==================================================================================
Combina descriptores densos de Deep Learning (DINOv2 ViT) con análisis morfológico
avanzado de OpenCV (aislamiento de tinta, densidad de trazos, color CIE-Lab).

- Inferencia en CPU multihilo (QThread).
- Exportación directa a CSV, MySQL y PostgreSQL.
- Sin dependencias de JSON.
"""
...
# ---------------------------------------------------------------------------
# Módulo de Inteligencia Artificial (DINOv2 ViT)
# ---------------------------------------------------------------------------
class DINOv2Extractor:
    """Extrae descriptores de textura periódica mediante autoatención de Vision Transformer."""
    def __init__(self):
    ...
    def get_embedding(self, rgb_numpy):
        """Genera embedding normalizado L2 de 384 dimensiones."""
...
# ---------------------------------------------------------------------------
# Descomposición Morfológica y Cromática (OpenCV)
# ---------------------------------------------------------------------------
def extract_morphological_signature(img_bgr, target_size=(72, 48)):
    """Aísla la tinta del fondo, evitando que el color domine a la textura."""
    if img_bgr is None or img_bgr.size == 0:
        return None

    norm = cv2.resize(img_bgr, target_size, interpolation=cv2.INTER_AREA)

    # Color mediano de fondo (CIE-Lab)
    lab = cv2.cvtColor(norm, cv2.COLOR_BGR2LAB)
    l_c, a_c, b_c = cv2.split(lab)
    bg_lab = np.array([np.median(l_c), np.median(a_c), np.median(b_c)], dtype=np.float32)

    # Detección de tinta por desviación respecto al fondo dominante
    dist_from_bg = np.sqrt(
        (l_c.astype(np.float32) - bg_lab[0]) ** 2 +
        (a_c.astype(np.float32) - bg_lab[1]) ** 2 +
        (b_c.astype(np.float32) - bg_lab[2]) ** 2
    )
    _, ink_mask = cv2.threshold(dist_from_bg.astype(np.uint8), 28, 255, cv2.THRESH_BINARY)

    # Respaldo adaptativo para imágenes de muy bajo contraste
    if np.sum(ink_mask > 0) < 30:
        gray = cv2.cvtColor(norm, cv2.COLOR_BGR2GRAY)
        ink_mask = cv2.adaptiveThreshold(
            gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11, 4
        )

    # Perfiles direccionales de tinta
    proj_x = np.sum(ink_mask, axis=0, dtype=np.float32) / (target_size[1] * 255.0)
    proj_y = np.sum(ink_mask, axis=1, dtype=np.float32) / (target_size[0] * 255.0)
    ...
    return {
        ...
        "proj_x": proj_x,
        "proj_y": proj_y,
        "ink_density": float(np.mean(ink_mask > 0))
    }


def compare_morphology(sig1, sig2):
    """Evalúa similitud de color y trama morfológica sin el sesgo de saturación."""
    if sig1 is None or sig2 is None:
        return 0.0, 0.0

    # Color de fondo
    delta_e = np.linalg.norm(sig1["bg_lab"] - sig2["bg_lab"])
    score_color = max(0.0, 100.0 - (delta_e * 1.6))
    if delta_e > 45.0:
        score_color *= 0.3

    # Trama morfológica
    m1, m2 = sig1["mask"], sig2["mask"]
    intersection = np.logical_and(m1 > 0, m2 > 0).sum()
    union = np.logical_or(m1 > 0, m2 > 0).sum()
    iou = (intersection / max(union, 1)) * 100.0

    corr_x = np.corrcoef(sig1["proj_x"], sig2["proj_x"])[0, 1]
    corr_y = np.corrcoef(sig1["proj_y"], sig2["proj_y"])[0, 1]
    if np.isnan(corr_x): corr_x = 0.0
    if np.isnan(corr_y): corr_y = 0.0
    score_proj = max(0.0, (corr_x * 0.4 + corr_y * 0.6)) * 100.0

    dens_diff = abs(sig1["ink_density"] - sig2["ink_density"])
    score_density = max(0.0, 100.0 - (dens_diff * 250.0))

    score_pattern = (iou * 0.40) + (score_proj * 0.35) + (score_density * 0.25)
    return score_color, score_pattern
...
# ---------------------------------------------------------------------------
# Worker Thread: Segmentación, OCR y Matching Híbrido Integral
# ---------------------------------------------------------------------------
class ProcessingWorker(QThread):
    ...
    def _process(self):
        ...
        self.progress.emit(10, "Detectando contornos...")
        ...
        self.progress.emit(20, f"{len(boxes)} segmentos encontrados.")

        if not boxes:
            self.error.emit("No se detectaron recuadros de patrones en la imagen.")
            return
        ...
        for i, (bx, by, bw, bh) in enumerate(boxes):
            ...
            self.progress.emit(pct, f"OCR elemento {i+1}/{total}...")
        ...
        # 4. Análisis Híbrido: DINOv2 + Morfología de Trama
        self.stage_changed.emit("Etapa 4: Ensamble IA (DINOv2) + Morfología de Trama")
        self.progress.emit(65, "Cargando pesos de DINOv2 e indexando catálogo...")
        ...
        for pf in pattern_files:
            ...
            if target.ndim == 3 and target.shape[2] == 4:
                ...
                base = cv2.cvtColor(target, cv2.COLOR_GRAY2BGR)
            else:
                base = target

            sig = extract_morphological_signature(base)
            target_rgb = cv2.cvtColor(base, cv2.COLOR_BGR2RGB)
            dino_emb = ai_extractor.get_embedding(target_rgb)

            if sig is not None and dino_emb is not None:
                catalog.append({
                    "filename": pf,
                    "sig": sig,
                    "dino_emb": dino_emb,
        ...
        self.progress.emit(80, "Calculando ensamble de similitud visual e IA...")
        for i, seg in enumerate(segments):
            ...
            seg_sig = extract_morphological_signature(pure_bgr)
            ...
            for cat_item in catalog:
                ...
                sc_color, sc_trama = compare_morphology(seg_sig, cat_item["sig"])

                # Componente de Inteligencia Artificial
                sc_dino = ai_extractor.cosine_similarity(seg_dino, cat_item["dino_emb"])

                # Ensamble Ponderado:
                # 45% DINOv2 (comprensión profunda de estructura/patrón)
                # 35% Morfología de Trama (bloques, rayas, densidad de tinta)
                # 20% Color de fondo LAB
                score_final = (sc_dino * 0.45) + (sc_trama * 0.35) + (sc_color * 0.20)
...
# ---------------------------------------------------------------------------
# Diálogo de Exportación (CSV, MySQL, PostgreSQL)
# ---------------------------------------------------------------------------
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Lith Analizador Litológico (Ensamble IA + Morfología)")
    ...
    def _build_ui(self):
        ...
        self.phase_btns = []
        for i, txt in enumerate(["① Configuración", "② Procesamiento Híbrido", "③ Resultados"]):
            b = QPushButton(txt)
            b.setFixedHeight(36)
            b.setEnabled(i == 0)
    ...
    def _build_phase1(self):
        ...
        self.lbl_csv_status = QLabel(
            f"✅ {len(self.geo_lith)} registros cargados" if self.geo_lith else "❌ No encontrado"
        )
        ...
        self.btn_process = QPushButton("▶  PROCESAR (IA + MORFOLOGÍA)")
    ...
    def _build_phase3(self):
        ...
        f3 = QFrame()
        l3 = QVBoxLayout(f3)
        l3.addWidget(QLabel("<b>Patrón Coincidente (Híbrido)</b>"), alignment=Qt.AlignCenter)
    ...
    def _start_processing(self):
        if not self.image_path:
            QMessageBox.warning(self, "Atención", "Cargá una imagen primero.")
            return
        if not self.geo_lith:
            QMessageBox.warning(self, "Atención", "No se encontró el archivo geo_lith.csv.")
            return
