"""
Lith_main.py – Analizador Litológico de Patrones (Motor Fiel + Desempate IA)
=============================================================================
Restaura exactamente la matemática y preprocesamiento del código original:
  - Margen fijo exacto de 2px (sin deformaciones)
  - Color CIE-Lab, histogramas BGR/HSV (6 canales), NCC y Laplaciano
  - Multi-escala nativa, normalizada (64x44) y reducida (48x33)
  - Candado cromático estricto (no mezcla colores de fondo)
  - IA DINOv2 sutil como desempate fino

Exportación: CSV, MySQL y PostgreSQL.
...
# ---------------------------------------------------------------------------
# Algoritmo de Comparación Fiel del Código Inicial
# ---------------------------------------------------------------------------
def _combined_image_score(patch_bgr, target_bgr):
    """Puntaje combinado original: color LAB, histogramas HSV, estructura NCC y bordes Laplacian."""
    ...
    # 1. Color LAB con candado estricto
    ...
    # Si la diferencia cromática media es abismal (ej. verde vs azul), el score es 0 directo
    if mean_diff > 38.0:
        return 0.0
...
def evaluate_pattern_original_multiscale(patch_bgr, target_variants, norm_size=(64, 44)):
    """Evalúa a escala nativa, normalizada y reducida como el script inicial."""
    best = 0.0
...
# ---------------------------------------------------------------------------
# Desempate Fino por Inteligencia Artificial (DINOv2)
# ---------------------------------------------------------------------------
class DINOv2TieBreaker:
    def __init__(self):
...
# ---------------------------------------------------------------------------
# Worker Thread: Segmentación, OCR y Matching Restaurado
# ---------------------------------------------------------------------------
class ProcessingWorker(QThread):
    ...
    def _process(self):
        ...
        self.progress.emit(10, "Detectando casillas...")
        ...
        if not boxes:
            self.error.emit("No se detectaron casillas litológicas.")
            return
        ...
        self.progress.emit(25, "Inicializando OCR...")
        ...
        for i, (bx, by, bw, bh) in enumerate(boxes):
            ...
            ph, pw = patch.shape[:2]
            # Margen idéntico al código original (2px)
            if ph > 8 and pw > 8:
        ...
        self.progress.emit(55, "Correlacionando códigos...")
        for seg in segments:
            id_lith, matched_name = match_lith_name(seg["ocr_text"], self.geo_lith)
            seg["idLith"] = id_lith
            seg["matched_nombre"] = matched_name

        # 4. Clasificación Multi-Métrica Original + IA Desempate
        self.stage_changed.emit("Etapa 4: Clasificación Visual y Desempate con IA")
        self.progress.emit(65, "Indexando catálogo de patrones y activando IA...")
        ai_extractor = DINOv2TieBreaker()
        ...
        for i, seg in enumerate(segments):
            ...
            for cat_item in catalog:
                # 1. Comparación multi-escala del código original evaluando pure_patch y patch
                s_cv = max(
                    evaluate_pattern_original_multiscale(pure_bgr, cat_item["variants"]),
                    evaluate_pattern_original_multiscale(patch_bgr, cat_item["variants"])
                )

                # Si el candado cromático o estructural descartó el patrón, saltar
                if s_cv <= 0.0:
                    continue

                # 2. Desempate fino por IA (solo si hay compatibilidad cromática previa)
                s_dino = ai_extractor.cosine_similarity(seg_dino, cat_item["dino_emb"])

                # Fórmula sólida: 85% la precisión clásica + 15% IA
                score_final = (s_cv * 0.85) + (s_dino * 0.15)
