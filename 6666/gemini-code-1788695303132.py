"""
Lith_main.py – Analizador Litológico de Patrones (Pipeline Clásico Fiel + IA DINOv2)
==================================================================================
Restaura la formulación matemática original multi-métrica y multi-escala:
  - Color perceptual CIE-Lab
  - Histogramas BGR + HSV en 6 canales
  - Correlación Estructural Normalizada (NCC)
  - Coincidencia de bordes con Laplaciano
  - Multi-escala nativa, normalizada y pequeña
Integrada con embeddings profundos DINOv2 (ViT-Small/14) como desempate morfológico.

Exportación: CSV, MySQL y PostgreSQL.
Sin JSON. Multihilo con PySide6.
"""
...
# ---------------------------------------------------------------------------
# Motor de Comparación Clásica Exacto del Código Original
# ---------------------------------------------------------------------------
...
def evaluate_pattern_multiscale(patch_bgr, target_variants, norm_size=(64, 44)):
    """Evalúa a escala nativa, normalizada y pequeña para variantes de fondo transparente."""
    best = 0.0
...
# ---------------------------------------------------------------------------
# Worker Thread: Segmentación, OCR y Matching Fiel Multi-Métrica + IA
# ---------------------------------------------------------------------------
class ProcessingWorker(QThread):
    ...
    def _process(self):
        ...
        self.progress.emit(10, "Detectando recuadros...")
        ...
        if not boxes:
            self.error.emit("No se detectaron recuadros litológicos válidos.")
            return
        ...
        # 2. OCR
        self.stage_changed.emit("Etapa 2: Extrayendo textos con EasyOCR")
        ...
        for i, (bx, by, bw, bh) in enumerate(boxes):
            ...
            ph, pw = patch.shape[:2]
            # Recorte interior fiel al código original
            if ph > 8 and pw > 8:
        ...
        # 4. Pipeline de Análisis Fiel + IA
        self.stage_changed.emit("Etapa 4: Comparación Multi-Métrica + IA")
        self.progress.emit(65, "Cargando motor de IA y catálogo de patrones...")
        ...
        self.progress.emit(75, "Evaluando similitudes...")
        for i, seg in enumerate(segments):
            pct = 75 + int((i / total) * 23)
            self.progress.emit(pct, f"Procesando patrón {i+1}/{total}...")
            ...
            for cat_item in catalog:
                ...
                # Ponderación calibrada: 75% OpenCV Clásico (alta precisión comprobada) + 25% DINOv2 (desempate)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Lith Analizador Litológico (Original Fiel + IA)")
    ...
    def _build_phase1(self):
        ...
        self.btn_process = QPushButton("▶  PROCESAR (MOTOR COMPLETO)")
