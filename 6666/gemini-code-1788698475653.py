"""
Lith_main_opcion1.py – Analizador Litológico con Cascada Semántica
==================================================================
1. EasyOCR + Fuzzy Match sobre geo_lith.csv.
2. Agrupación por familia litológica: restringe la búsqueda a candidatos válidos.
3. Motor visual clásico multiescala (LAB, BGR/HSV, NCC, Laplaciano) para desempate.
...
def get_lithology_family(name):
    """Clasifica el nombre geológico en una familia dominante."""
    n = name.upper()
    if any(k in n for k in ["CALIZA", "LIME"]): return "CALIZA"
    if any(k in n for k in ["DOLOMITA", "DOLO"]): return "DOLOMITA"
    if any(k in n for k in ["ARENISCA", "SAND"]): return "ARENISCA"
    if any(k in n for k in ["ARCILITA", "LUTITA", "ARCILL", "SHALE"]): return "ARCILITA"
    if any(k in n for k in ["TOBA", "TUFF"]): return "TOBA"
    if any(k in n for k in ["CONGLOMERADO", "CONGL"]): return "CONGLOMERADO"
    if any(k in n for k in ["YESO", "ANHIDRITA"]): return "EVAPORITA"
    return "OTRO"

def get_catalog_family(bgr_img):
    """Determina la familia geológica del patrón de referencia por su color dominante."""
    lab = cv2.cvtColor(bgr_img, cv2.COLOR_BGR2LAB)
    l, a, b = cv2.split(lab)
    med_l, med_a, med_b = np.median(l), np.median(a), np.median(b)
    # Azul: b bajo
    if med_b < 118: return "CALIZA"
    # Amarillo: b alto, l alto
    if med_b > 145 and med_l > 120: return "ARENISCA"
    # Verde: a bajo, b medio-alto
    if med_a < 120: return "ARCILITA"
    # Gris o tonos neutros
    if abs(med_a - 128) < 8 and abs(med_b - 128) < 8: return "TOBA"
    return "OTRO"
...
class ProcessingWorker(QThread):
    ...
    def _process(self):
        ...
        if not boxes:
            self.error.emit("No se detectaron recuadros de patrones.")
            return
        ...
        for seg in segments:
            ...
            seg["family"] = get_lithology_family(matched_name)

        self.stage_changed.emit("Etapa 4: Comparación por Familia y Multiescala")
        ...
        for pf in pattern_files:
            ...
            fam = get_catalog_family(variants[0])
            catalog.append({"filename": pf, "variants": variants, "family": fam})

        self.progress.emit(75, "Evaluando patrones acotados...")
        for i, seg in enumerate(segments):
            pct = 75 + int((i / total) * 23)
            self.progress.emit(pct, f"Evaluando patrón {i+1}/{total}...")

            pure_bgr = cv2.cvtColor(seg["pure_patch"], cv2.COLOR_RGB2BGR)
            patch_bgr = cv2.cvtColor(seg["patch"], cv2.COLOR_RGB2BGR)
            target_fam = seg["family"]

            # Filtro: evaluar preferentemente patrones de la misma familia
            fam_candidates = [c for c in catalog if c["family"] == target_fam]
            eval_list = fam_candidates if len(fam_candidates) >= 2 else catalog

            best_file, best_score = "", 0.0
            for cat_item in eval_list:
                s = max(
            ...
            # Respaldo si no hubo coincidencia en la familia
            if best_score < 30.0 and eval_list is not catalog:
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Lith Analizador Litológico – Cascada Semántica (Opción 1)")
