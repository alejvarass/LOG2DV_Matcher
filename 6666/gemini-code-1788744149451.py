"""
Lith_main.py – Analizador Litológico de Patrones (Motor Calibrado de Alta Precisión)
===================================================================================
Pipeline híbrido con desempate morfológico por componentes internos:
  - Clasificación OCR y correspondencia con geo_lith.csv.
  - Análisis multi-métrica y multi-escala fiel de OpenCV (LAB, BGR/HSV, NCC, Laplaciano).
  - Discriminación por componentes secundarios (líneas de arcilita vs puntos de arenisca).
  - Embeddings de Vision Transformer (DINOv2) para regularización visual.
...
# ---------------------------------------------------------------------------
# Motor Visual Clásico Multi-Métrica
# ---------------------------------------------------------------------------
def _combined_image_score(patch_bgr, target_bgr):
    ...
    score_color = max(0.0, 100.0 - mean_diff * 1.3)
...
# ---------------------------------------------------------------------------
# Extractor de Signos Morfológicos Secundarios (Puntos vs Rayas)
# ---------------------------------------------------------------------------
def compute_ink_traits(bgr_img):
    """Evalúa si el contenido interno contiene principalmente líneas o puntos."""
    if bgr_img is None or bgr_img.size == 0:
        return {"has_lines": False, "has_dots": False, "horiz_ratio": 0.0}
    
    gray = cv2.cvtColor(bgr_img, cv2.COLOR_BGR2GRAY)
    _, bin_inv = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    
    # Kernel horizontal para detectar rayas de arcilita / calcáreas
    h_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (7, 1))
    lines = cv2.morphologyEx(bin_inv, cv2.MORPH_OPEN, h_kernel)
    line_pixels = int(np.sum(lines > 0))
    
    # Kernel pequeño para detectar punteado de arenisca
    dot_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
    dots = cv2.morphologyEx(bin_inv, cv2.MORPH_OPEN, dot_kernel)
    total_ink = max(1, int(np.sum(bin_inv > 0)))
    
    horiz_ratio = line_pixels / total_ink
    return {
        "has_lines": horiz_ratio > 0.28,
        "has_dots": horiz_ratio < 0.18,
        "horiz_ratio": horiz_ratio
    }

# ---------------------------------------------------------------------------
# Desempate con DINOv2
# ---------------------------------------------------------------------------
...
class ProcessingWorker(QThread):
    ...
    def _process(self):
        ...
        self.stage_changed.emit("Etapa 4: Clasificación Visual y Morfológica")
        ...
        for pf in pattern_files:
            ...
            traits = compute_ink_traits(variants[0])
        ...
        for i, seg in enumerate(segments):
            ...
            seg_traits = compute_ink_traits(pure_bgr)
            name_upper = seg["matched_nombre"].upper()
            ...
            for cat_item in catalog:
                ...
                score_base = (s_cv * 0.75) + (s_dino * 0.25)

                # Desempate morfológico contextual para litologías compuestas
                morph_bonus = 1.0
                cat_traits = cat_item["traits"]

                # 1. Si es Caliza Arcillosa o Arcilita, premiar patrones con líneas horizontales
                if any(k in name_upper for k in ["ARCILLOSA", "LUTITA", "ARCILITA"]):
                    if cat_traits["has_lines"] and seg_traits["has_lines"]:
                        morph_bonus = 1.08
                    elif cat_traits["has_dots"]:
                        morph_bonus = 0.92

                # 2. Si es Arenosa o Arenisca, premiar patrones punteados
                if any(k in name_upper for k in ["ARENOSA", "ARENISCA"]) and "CALCAREA" not in name_upper:
                    if cat_traits["has_dots"]:
                        morph_bonus = 1.08

                # 3. Si es Arenisca Calcárea, verificar presencia simultánea de líneas/bloques y puntos
                if "CALCAREA" in name_upper and "ARENISCA" in name_upper:
                    if cat_traits["has_lines"] or cat_traits["horiz_ratio"] > 0.20:
                        morph_bonus = 1.10

                score_final = score_base * morph_bonus
