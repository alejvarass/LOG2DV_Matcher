"""
Lith_main.py – Analizador Litológico de Patrones (Motor Híbrido Integral con Código Litológico)
=============================================================================================
Segmenta leyendas litológicas, ejecuta EasyOCR, correlaciona con geo_lith.csv y
clasifica el patrón gráfico combinando:
  - Recorte dinámico de marco perimetral negro (Auto-Cropping)
  - Análisis multi-métrica y multi-escala fiel de OpenCV (LAB, BGR/HSV, NCC, Laplaciano)
  - Embeddings de Vision Transformer (DINOv2 ViT-Small/14)
  - Priorización inteligente por Código Litológico (idLith)

Exportación multibase: CSV, MySQL y PostgreSQL.
Sin persistencia JSON. Ejecución multihilo no bloqueante en PySide6.
"""
...
def list_pattern_files(patterns_dir):
    """Lista ordenada de archivos gráficos dentro del directorio."""
...
# ---------------------------------------------------------------------------
# Estilos Dark Theme (Catppuccin Macchiato)
# ---------------------------------------------------------------------------
DARK_STYLE = """
QMainWindow, QWidget {
...
# ---------------------------------------------------------------------------
# Limpieza de bordes y auto-recorte morfológico
# ---------------------------------------------------------------------------
def clean_patch_content(patch_bgr):
    """Detecta y remueve el borde negro perimetral para aislar la textura."""
    ...
    if contours:
        ...
        if w > 12 and h > 10:
            return np.ascontiguousarray(patch_bgr[y+2:y+h-2, x+2:x+w-2])

    ph, pw = patch_bgr.shape[:2]
    return np.ascontiguousarray(patch_bgr[3:ph-3, 3:pw-3]) if ph > 10 and pw > 10 else patch_bgr


# ---------------------------------------------------------------------------
# Motor Visual Clásico Multi-Métrica y Multi-Escala
# ---------------------------------------------------------------------------
def _combined_image_score(patch_bgr, target_bgr):
    """Puntaje combinado fiel: LAB, histogramas BGR/HSV, NCC y bordes Laplacian."""
...
def evaluate_pattern_multiscale(patch_bgr, target_variants, norm_size=(64, 44)):
    """Evalúa a escala nativa, normalizada y reducida."""
    best = 0.0
...
# ---------------------------------------------------------------------------
# Motor de Inteligencia Artificial (DINOv2)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Worker Thread: Segmentación, OCR y Matching Híbrido Calibrado
# ---------------------------------------------------------------------------
class ProcessingWorker(QThread):
    ...
    def _process(self):
        # 1. Segmentación
        self.stage_changed.emit("Etapa 1: Segmentando imagen litológica")
        ...
        self.progress.emit(10, "Detectando contornos de casillas...")
        ...
        self.progress.emit(20, f"{len(boxes)} casillas encontradas.")

        if not boxes:
            self.error.emit("No se detectaron casillas de leyendas en la imagen.")
            return
        ...
        self.progress.emit(55, "Correlacionando códigos en geo_lith.csv...")
        for seg in segments:
            id_lith, matched_name = match_lith_name(seg["ocr_text"], self.geo_lith)
            seg["idLith"] = id_lith
            seg["matched_nombre"] = matched_name

        # 4. Clasificación Multi-Métrica + DINOv2 con Prioridad de Código
        self.stage_changed.emit("Etapa 4: Clasificación Visual Híbrida e IA")
        self.progress.emit(65, "Cargando DINOv2 e indexando patrones...")
        ...
        for pf in pattern_files:
            ...
            # Extraer posibles números en el nombre de archivo (ej: 8 de '4-8.png' o '8.png')
            nums_in_file = set(re.findall(r"\d+", pf))
        ...
        self.progress.emit(75, "Evaluando patrones limpios...")
        for i, seg in enumerate(segments):
            pct = 75 + int((i / total) * 23)
            self.progress.emit(pct, f"Procesando elemento {i+1}/{total}...")

            # Limpiar borde perimetral negro antes de comparar
            clean_bgr = clean_patch_content(cv2.cvtColor(seg["patch"], cv2.COLOR_RGB2BGR))
            clean_rgb = cv2.cvtColor(clean_bgr, cv2.COLOR_BGR2RGB)
            seg_dino = ai_extractor.get_embedding(clean_rgb)

            id_str = str(seg["idLith"])
            best_file = ""
            best_score = 0.0

            for cat_item in catalog:
                # 1. Comparación multi-escala fiel
                s_cv = evaluate_pattern_multiscale(clean_bgr, cat_item["variants"])

                # 2. Comparación DINOv2
                s_dino = ai_extractor.cosine_similarity(seg_dino, cat_item["dino_emb"])

                # 3. Ensamble Base: 70% Motor Clásico + 30% IA
                score_base = (s_cv * 0.70) + (s_dino * 0.30)

                # 4. Ponderación por Código Litológico (idLith)
                # Si el idLith detectado por OCR está contenido en el nombre del archivo de patrón
                # se le otorga un impulso definitivo que evita confusiones de texturas hermanas
                bonus = 1.0
                if seg["idLith"] != -1 and id_str in cat_item["nums"]:
                    bonus = 1.25

                score_final = score_base * bonus
            ...
            seg["pattern_score"] = round(min(100.0, best_score), 2)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Lith Analizador Litológico (IA + Motor Morfológico)")
