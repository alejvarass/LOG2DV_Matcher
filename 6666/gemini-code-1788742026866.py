"""
Lith_main.py – Analizador Litológico de Patrones (Ajuste Conglomerados, Calcáreas y Tobas)
========================================================================================
Optimizada la detección de litologías compuestas (calcáreas, tobas y conglomerados).
Combina análisis morfológico de OpenCV con desempate DINOv2.

Exportación: CSV, MySQL y PostgreSQL.
Sin persistencia JSON. Multihilo con PySide6.
"""
...
def match_lith_name(ocr_text, geo_lith_records):
    ...
    matches = get_close_matches(upper, names, n=1, cutoff=0.42)
    if matches:
    ...
    for i, n in enumerate(names):
        ...
        if s > best_score:
            best_score = s
            best_idx = i
    if best_score >= 0.32 and best_idx >= 0:
        return geo_lith_records[best_idx]["idLith"], geo_lith_records[best_idx]["nombre"]
    return -1, ocr_text

def get_lithology_tags(name):
    """Devuelve un conjunto de etiquetas para litologías simples o compuestas."""
    n = name.upper()
    tags = set()
    if any(k in n for k in ["CALIZA", "CALCAREA", "CALC", "LIME"]): tags.add("CALIZA")
    if any(k in n for k in ["DOLOMITA", "DOLO"]): tags.add("DOLOMITA")
    if any(k in n for k in ["ARENISCA", "AREN", "SAND"]): tags.add("ARENISCA")
    if any(k in n for k in ["ARCILITA", "ARCILLOSA", "LUTITA", "ARCILL", "SHALE"]): tags.add("ARCILITA")
    if any(k in n for k in ["TOBA", "TOBACEA", "TOB", "TUFF"]): tags.add("TOBA")
    if any(k in n for k in ["CONGLOMERADO", "CONGL"]): tags.add("CONGLOMERADO")
    if any(k in n for k in ["LIMO", "LIMOLITA"]): tags.add("LIMOLITA")
    return tags
...
def _combined_image_score(patch_bgr, target_bgr):
    ...
    # 1. Color LAB: penalización gradual en vez de corte a cero brusco
    ...
    if mean_diff > 60.0: 
        return 0.0
    score_color = max(0.0, 100.0 - mean_diff * 1.5)
...
class ProcessingWorker(QThread):
    ...
    def _process(self):
        ...
        for seg in segments:
            ...
            seg["tags"] = get_lithology_tags(matched_name)

        self.stage_changed.emit("Etapa 4: Clasificación Híbrida y Desempate con IA")
        self.progress.emit(65, "Indexando catálogo de patrones y activando DINOv2...")
        ...
        for pf in pattern_files:
            ...
            catalog.append({"filename": pf, "variants": variants, "dino_emb": dino_emb})
        ...
        for i, seg in enumerate(segments):
            ...
            for cat_item in catalog:
                ...
                # Fusión ponderada: 80% motor clásico multiescala + 20% DINOv2
                score_final = (s_cv * 0.80) + (s_dino * 0.20)
