"""
Lith_main.py – Analizador Litológico de Patrones (Enfoque Mixto Integral)
==========================================================================
Combina la batería de análisis morfológico-cromático clásico de OpenCV:
  - Distancia Delta-E en espacio CIE-Lab
  - Histogramas cruzados BGR y HSV (6 canales)
  - Coeficiente de correlación normalizada (NCC)
  - Coincidencia de bordes con Laplaciano
  - Análisis multi-escala (nativa, 64x44, 48x33)

Con Inteligencia Artificial densa:
  - Embeddings de Vision Transformer (DINOv2 ViT-Small/14)
  - Similitud de Coseno en hiperesfera unitaria

Exportación multibase: CSV, MySQL y PostgreSQL.
Sin persistencia JSON. Ejecución multihilo en CPU con PySide6.
"""
...
# ---------------------------------------------------------------------------
# Motor de Comparación Clásica de OpenCV (Preservado del Código Original)
# ---------------------------------------------------------------------------
def _combined_image_score(patch_bgr, target_bgr):
    ...
    # 1. Diferencia cromática perceptual en espacio LAB
    ...
    # 2. Histogramas multicanal BGR y HSV
    score_hist = 0.0
    for src, dst in (
...
def evaluate_cv_multiscale(patch_bgr, target_variants, norm_size=(64, 44)):
    """Evalúa la imagen en escala nativa, normalizada y pequeña para variantes de fondo."""
    best = 0.0
...
class DINOv2Extractor:
    ...
    def cosine_similarity(emb1, emb2):
        """Similitud coseno de vectores normalizados en escala de 0 a 100."""
...
class ProcessingWorker(QThread):
    ...
    def _process(self):
        ...
        self.progress.emit(10, "Localizando contornos...")
        ...
        self.progress.emit(20, f"{len(boxes)} segmentos localizados.")

        if not boxes:
            self.error.emit("No se detectaron segmentos de patrones litológicos.")
            return
        ...
        self.progress.emit(25, "Cargando motor EasyOCR...")
        ...
        for i, (bx, by, bw, bh) in enumerate(boxes):
            ...
            self.progress.emit(pct, f"Extrayendo texto {i+1}/{total}...")

            patch = img_np[by:by+bh, bx:bx+bw]

            # Remoción del borde perimetral para aislar la textura limpia
            ph, pw = patch.shape[:2]
            if ph > 8 and pw > 8:
        ...
        # --- Etapa 3: Identificación de texto en CSV ---
        self.stage_changed.emit("Etapa 3: Correlación litológica con geo_lith.csv")
        self.progress.emit(55, "Correlacionando nombres...")
        for seg in segments:
            id_lith, matched_name = match_lith_name(seg["ocr_text"], self.geo_lith)
            seg["idLith"] = id_lith
            seg["matched_nombre"] = matched_name

        # --- Etapa 4: Clasificación Híbrida Integral (OpenCV Clásico + DINOv2) ---
        self.stage_changed.emit("Etapa 4: Análisis Híbrido Integral (Color/Estructura + DINOv2)")
        self.progress.emit(65, "Inicializando DINOv2...")
        ai_extractor = DINOv2Extractor()

        self.progress.emit(70, "Preprocesando catálogo de patrones...")
        ...
        for pf in pattern_files:
            ...
            # Generar variantes de canal alfa para OpenCV
            variants = []
            ...
            # Extraer embedding DINOv2 de la variante principal (RGB)
        ...
        self.progress.emit(80, "Ejecutando ensamble de similitud visual...")
        for i, seg in enumerate(segments):
            ...
            # Formatos BGR para el pipeline OpenCV
            pure_bgr = cv2.cvtColor(pure_rgb, cv2.COLOR_RGB2BGR)
            patch_bgr = cv2.cvtColor(patch_rgb, cv2.COLOR_RGB2BGR)

            # Embeddings DINOv2 para el pipeline de IA
            seg_dino = ai_extractor.get_embedding(pure_rgb)
            if seg_dino is None:
                seg_dino = ai_extractor.get_embedding(patch_rgb)

            best_file = ""
            best_score = 0.0

            for cat_item in catalog:
                # 1. Puntuación clásica OpenCV: evalúa parches limpios y enteros en multiescala
                score_cv = max(
                    evaluate_cv_multiscale(pure_bgr, cat_item["variants"]),
                    evaluate_cv_multiscale(patch_bgr, cat_item["variants"])
                )

                # 2. Puntuación de Inteligencia Artificial (DINOv2)
                score_dino = ai_extractor.cosine_similarity(seg_dino, cat_item["dino_emb"])

                # 3. Ensamble balanceado:
                # 65% clásico (preserva la alta precisión en tonos LAB, histogramas y bordes)
                # 35% DINOv2 (desempata orientaciones y patrones geométricos)
                score_final = (score_cv * 0.65) + (score_dino * 0.35)
...
# ---------------------------------------------------------------------------
# Módulo de Exportación Multibase (CSV, MySQL, PostgreSQL)
# ---------------------------------------------------------------------------
class ExportDialog(QDialog):
    ...
    def _export_csv(self, path):
        try:
            ...
            QMessageBox.information(self, "Exportado", f"CSV guardado correctamente en:\n{path}")
            self.accept()
        except Exception as e:
    ...
    def _export_mysql(self, path):
        try:
            ...
            with open(path, "w", encoding="utf-8") as f:
                f.write("\n".join(lines) + "\n")
            QMessageBox.information(self, "Exportado", f"Script MySQL guardado en:\n{path}")
            self.accept()
        except Exception as e:
            QMessageBox.critical(self, "Error", str(e))
    ...
    def _export_postgresql(self, path):
        try:
            ...
            with open(path, "w", encoding="utf-8") as f:
                f.write("\n".join(lines) + "\n")
            QMessageBox.information(self, "Exportado", f"Script PostgreSQL guardado en:\n{path}")
            self.accept()
        except Exception as e:
            QMessageBox.critical(self, "Error", str(e))
...
# ---------------------------------------------------------------------------
# Dropzone de Selección de Imagen
# ---------------------------------------------------------------------------
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Lith Analizador Litológico (Ensamble OpenCV + DINOv2)")
    ...
    def _build_phase1(self):
        ...
        self.btn_process = QPushButton("▶  PROCESAR (ENSAMBLE COMPLETO)")
    ...
    def _build_phase2(self):
        ...
        grp_log = QGroupBox("Log de Ejecución")
