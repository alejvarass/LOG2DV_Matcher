...
# ---------------------------------------------------------------------------
# Persistent results store & Base64 serialization (L1 features)
# ---------------------------------------------------------------------------
...
def _result_to_store_row(r):
    """Convert a live result dict to a serializable store row (safe from numpy types)."""
    row = {
...
# ---------------------------------------------------------------------------
# Image comparison: L2 algorithms preserved without modifications
# ---------------------------------------------------------------------------
def _lab_deltae_edges_score(patch_bgr, target_bgr, interp):
    """Score: CIE L*a*b* Delta E colour + Laplacian edges."""
...
def _hist_ncc_lab_score(patch_bgr, target_bgr, interp):
    """Score: HSV histograms + NCC structure + LAB difference."""
    h, w = patch_bgr.shape[:2]
    if h < 5 or w < 5:
        return 0.0
    target_resized = cv2.resize(target_bgr, (w, h), interpolation=interp)
...
def _combined_image_score(patch_bgr, target_bgr):
    """L2 Combined score combining color, histogram, structure and edges."""
...
# ---------------------------------------------------------------------------
# Fuzzy match OCR text to geo_lith nombre (L2 version intact)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Worker thread: segmentation + OCR + matching (L2 pipeline)
# ---------------------------------------------------------------------------
class ProcessingWorker(QThread):
    ...
    def _process(self):
        ...
        # --- Stage 4: Pattern matching (L2 logic) ---
...
# ---------------------------------------------------------------------------
# Main Window (Full PySide UI with Table Combo / Store / Sync from L1)
# ---------------------------------------------------------------------------
